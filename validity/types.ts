...
export interface TBase { readonly tag: 'base'; readonly name: BaseName }
export interface TVar  { readonly tag: 'var';  readonly id: number; readonly hint?: string }
export interface TCont { readonly tag: 'cont'; readonly body: readonly Instr[] }
...
export const unify = (a: Type, b: Type, s: Subst) => {
    ...
    if (aa.tag === 'base' && bb.tag === 'base') {
        if (bb.name === "any") return; // allow any
        // both base
        if (aa.name !== bb.name) {
